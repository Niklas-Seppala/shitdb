...
static void create_new_root(SDBTable *table, uint32_t right_child_page_num) {
    ...
    */
    ...
    // Clear root bit.
    left_child->type &= ~SDB_ROOT_NODE;

    root->type = SDB_INTERNAL_NODE | SDB_ROOT_NODE;
    root->body.internal.num_keys = 1;
    *internal_node_child(root, 0) = left_child_page_num;
    uint32_t left_child_max_key = get_node_max_key(left_child);
    root->body.internal.children[0].key = left_child_max_key;
    root->body.internal.right_child = right_child_page_num;
}
...
insert id=0 username=a email=a
    ...
    */
    ...
    old_node->body.leaf.num_cells = LEAF_NODE_LEFT_SPLIT_COUNT;
    new_node->body.leaf.num_cells = LEAF_NODE_RIGHT_SPLIT_COUNT;

    if (old_node->type & SDB_ROOT_NODE) {
...
void leaf_node_insert(SDBCursor* cursor, uint32_t key, SDBRow* value) {
    ...
    if (cursor->cell_num < num_cells) {
        ...
        for (uint32_t i = num_cells; i > cursor->cell_num; i--) {
            memcpy(& page->body.leaf.cells[i], & page->body.leaf.cells[i-1], LEAF_NODE_CELL_SIZE);
        }
    }

    page->body.leaf.num_cells++;
    page->body.leaf.cells[cursor->cell_num].key = key;
    char *dest = page->body.leaf.cells[cursor->cell_num].value;
...
ExecuteResult sdb_insert_execute(SDBStatement *statement, SDBTable *table) {
    ...
    if (cursor.cell_num < page->body.leaf.num_cells) {
        uint32_t key_at = page->body.leaf.cells[cursor.cell_num].key;
        if (key_at == key) {
...
void print_tree(SDBPager* pager, uint32_t page_num, uint32_t indentation_level) {
    ...
    if (node->type & SDB_LEAF_NODE) {
        num_keys = node->body.leaf.num_cells;
        indent(indentation_level);
        printf("- leaf (size %d)\n", num_keys);
        for (uint32_t i = 0; i < num_keys; i++) {
            indent(indentation_level + 1);
            printf("- %d\n", node->body.leaf.cells[i].key);
        }
    }

    if (node->type & SDB_INTERNAL_NODE) {
        num_keys = node->body.internal.num_keys;
        indent(indentation_level);
        ...
        for (uint32_t i = 0; i < num_keys; i++) {
            ...
            indent(indentation_level + 1);
            printf("- key %d\n", node->body.internal.children[i].key);
        }
        child = node->body.internal.right_child;
        print_tree(pager, child, indentation_level + 1);
    }
}
...
void sdb_cursor_start(SDBCursor *cursor, SDBTable *table) {
    ...
    cursor->end_of_table = (leaf_node->num_cells == 0);
}
...
static void leaf_node_find(SDBCursor *cursor, SDBTable* table, uint32_t page_num, uint32_t key) {
    ...
    uint32_t one_past_max_index = leaf_node->num_cells;

    while (one_past_max_index != min_index) {
        uint32_t index = (min_index + one_past_max_index) / 2;
        uint32_t key_at_index = leaf_node->cells[index].key;
        if (key == key_at_index) {
...
char *sdb_cursor_value(SDBCursor *cursor) {
    ...
    return node->cells[cursor->cell_num].value;
}
...
void sdb_cursor_advance(SDBCursor *cursor) {
    ...
    assert(cursor->cell_num <= node->num_cells && "Skipping cell");
    cursor->cell_num++;
